...
//Debug Line Interface
void eae6320::Graphics::DebugLine::LoadDebugLine()
{
	//create the vertices using shapevertex
	...
	const unsigned int bufferSize = vertexCount * sizeof(ShapeVertex);
	DWORD usage = 0;
...
{
	Graphics::GetLocalDirect3dDevice()->SetStreamSource(0, m_vBuffer, 0, sizeof(ShapeVertex));
...
//Debug Sphere Interface
void eae6320::Graphics::DebugSphere::LoadDebugSphere()
{
	std::vector<ShapeVertex> i_vertexData;
	std::vector<unsigned int> i_indexData;

	i_vertexData.push_back(ShapeVertex(0.0f, m_radius, 0.0f, (uint8_t)(m_color.z * 255), (uint8_t)(m_color.y * 255), (uint8_t)(m_color.x * 255)));
	...
	for (size_t i = 1; i <= m_stacks - 1; ++i) {
		...
		for (size_t j = 0; j <= m_slices; ++j) {
			...
			i_vertexData.push_back(ShapeVertex((m_radius * sin(phi) * cos(theta)),
				(m_radius * cos(phi)),
	...
	ShapeVertex *vertices = new ShapeVertex[m_vertexCount];
	...
	// create a vertex buffer interface called v_buffer
	const unsigned int bufferSize = m_vertexCount * sizeof(ShapeVertex);
	DWORD usage = 0;
	// We will define our own vertex format
	const DWORD useSeparateVertexDeclaration = 0;
	// Place the vertex buffer into memory that Direct3D thinks is the most appropriate
	...
	memcpy(pVoid, vertices, m_vertexCount * sizeof(ShapeVertex));
	m_vBuffer->Unlock();
