...
bool eae6320::Graphics::Texture::LoadTexture()
{
#ifdef EAE6320_PLATFORM_D3D
	{
		...
		const HRESULT result = D3DXCreateTextureFromFileEx(Graphics::GetLocalDirect3dDevice(), o_texturePath, useDimensionsFromFile, useDimensionsFromFile, useMipMapsFromFile,
...
{
	{
		...
		// Open the texture file
		{
			...
			fileHandle = CreateFile(o_texturePath, desiredAccess, otherProgramsCanStillReadTheFile,
		...
		// Verify that the file is a valid DDS
		{
			...
			else
			{
				...
				std::stringstream errorMessage;
				errorMessage << "The texture file \"" << o_texturePath << "\" isn't a valid DDS. The Four CC is \"" << fourCcString << "\""
					" instead of \"DDS \"";
				UserOutput::Print(errorMessage.str());
				goto OnExit;
			}
		...
		GLenum format;
		{
			...
			else
			{
				...
				std::stringstream errorMessage;
				errorMessage << "The texture file \"" << o_texturePath << "\" has an unsupported format \"" << fourCcString << "\"";
				UserOutput::Print(errorMessage.str());
				goto OnExit;
			}
		}
